...
public class LocalStack extends Stack {
    ...
    public LocalStack(final App scope, final String id, final StackProps props) {
        ...
        FargateService authService = createFargateService("AuthService", "auth-service", List.of(4005), authServiceDb, Map.of("JWT_SECRET", "YJKd5PWuYxjYyv5AT+ooMgYixqoRFBR4DsT8/7a50kY="));
    ...
    private CfnCluster createCluster() {
        return CfnCluster.Builder.create(this, "MskCluster").clusterName("kafka-cluster").kafkaVersion("2.8.0").numberOfBrokerNodes(1)
                //this just connects VPC to all brokernodes using the private subnet on VPC
                .brokerNodeGroupInfo(CfnCluster.BrokerNodeGroupInfoProperty.builder().instanceType("kafka.m5.xlarge").clientSubnets(vpc.getPrivateSubnets().stream().map(ISubnet::getSubnetId).collect(Collectors.toList()))
                        //specify how the brokers get distributed across our diff availability zones
                        .brokerAzDistribution("DEFAULT").build()).build();
    }

    //service discovery. i.e: auth-service.patient-management.local
    ...
    // For any internal services that we want to create
    private FargateService createFargateService(String id, String imageName, List<Integer> ports, DatabaseInstance db, Map<String, String> additionalEnvVars) {
        ...
        ContainerDefinitionOptions.Builder containerOptions = ContainerDefinitionOptions.builder().image(ContainerImage.fromRegistry(imageName))
                // this is what port should like
                .portMappings(ports.stream().map(port -> PortMapping.builder().containerPort(port).hostPort(port).protocol(Protocol.TCP).build()).toList()).logging(LogDriver.awsLogs(AwsLogDriverProps.builder().logGroup(LogGroup.Builder.create(this, id + "LogGroup").logGroupName("/ecs/" + imageName).removalPolicy(RemovalPolicy.DESTROY).retention(RetentionDays.ONE_DAY).build()).streamPrefix(imageName).build()));
    ...
    // Only for creating API gateway service as well as creating the load balancer
    private void createApiGatewayService() {
        ...
        // this one is automatically create an app load balancer
        ApplicationLoadBalancedFargateService apiGateway
                ...
                .desiredCount(1)
                // Decide how long the load balancer will wait for the api gateway container to start before it throws error
